            ...
            {posts.map(p=>{const sc=SC[p.status]||SC.draft;return(<div key={p.id} className="card" style={{display:'flex',gap:'1rem',alignItems:'flex-start'}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:'flex',gap:'1rem',marginTop:'.35rem'}}>
                  ...
                  <span className="text-xs text-muted">📅 {localDate(p.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span>
                  {p.published_at&&<span className="text-xs text-muted">🚀 {localDate(p.published_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span>}
                </div>
              </div>
