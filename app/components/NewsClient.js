  ...
  return (
    <>
      ...
      <div className="shell">
        ...
        {posts.length === 0 ? (
          <div className="empty-state"><div className="icon">📰</div><p>No news posts yet.</p></div>
        ) : filtered.length === 0 ? (
          <div className="empty-state"><div className="icon">🔍</div><p>No posts match your search.</p></div>
        ) : (
          <>
            {featured && <div style={{ marginBottom: '2rem' }}><NewsCard post={featured} featured /></div>}
            {rest.length > 0 && <div className="grid-3">{rest.map(p => <NewsCard key={p.id} post={p} />)}</div>}
          </>
        )}
      </div>
