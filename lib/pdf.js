...
async function getLaunchOptions() {
  // Vercel / Linux serverless — use bundled @sparticuz/chromium (no runtime download)
...
export function inlineLocalImages(html) {
...
export async function generateReceiptPdf(htmlContent) {
  ...
  try {
    ...
    const pdf = await page.pdf({
      ...
      scale: 0.75,
      margin: { top: '0.4cm', bottom: '0.4cm', left: '0.8cm', right: '0.8cm' },
    });
    return pdf;
  } finally {
    await browser.close();
