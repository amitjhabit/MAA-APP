...
export async function autoGenerateAndSendReceipt({
  ...
  try {
    ...
    const vars = {
      ...
      logo_img:            `<img src="/images/gallery/Mithila_logo.jpeg" alt="MAA Logo" style="width:80px;height:80px;border-radius:50%;object-fit:cover;display:block;margin:0 auto 12px;border:3px solid #E8720C;">`,
    };

    // Full receipt HTML — stored in DB and used for PDF
    const receiptHtml = inlineLocalImages(renderTemplate(tmpl.body_html || '', vars));

    // Logo as inline CID attachment — renders in all email clients without external URL blocking
